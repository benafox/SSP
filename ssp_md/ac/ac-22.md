...
x-trestle-set-params:
  ...
  ac-22_odp:
    ...
    ssp-values:
    - not applicable - the system has no publicly accessible content
    profile-param-value-origin: AC-22 implementation statement
x-trestle-global:
  profile:
...
All content on the system is restricted only to the parties directly involved, nothing is publicly accessible. All access requires authentication, IdP service with MFA for staff, credentialed access for parents, and bastion server for system administrators. Because there is no public-facing content, there are no designated publishers, pre-publication reviews, or published-content reviews to perform.
