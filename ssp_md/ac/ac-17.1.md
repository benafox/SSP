...
The system uses AWS SSM Session Manager to monitor privileged administrator sessions, which are then sent to the Splunk pipeline where the ISSO reviews them. AWS security group rules restrict inbound bastion server access to a source-IP allowlist. The app only allows TLS 1.2+ connections and denies all lower encryption methods.
