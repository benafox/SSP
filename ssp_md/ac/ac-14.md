...
x-trestle-set-params:
  ...
  ac-14_odp:
    ...
    ssp-values:
    - access to the login page only; no system function is permitted without authentication
    profile-param-value-origin: AC-14 implementation statement
x-trestle-global:
  profile:
...
There are no permitted actions on the system without identification or authentication. All system functions require an authenticated user. Unauthenticated users can only access the login page, and may not interact with the system in any way before submitting valid user credentials.
