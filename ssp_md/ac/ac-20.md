...
x-trestle-set-params:
  ...
  ac-20_odp.01:
    ...
    profile-param-value-origin: AC-20 implementation statement
    ssp-values:
  ...
  ac-20_odp.02:
    ...
    profile-param-value-origin: AC-20 implementation statement
    ssp-values:
    - TLS 1.2+ connections, multifactor authentication through the external school IdP, role-based access, and no export or local storage of organization data per the IPS Acceptable Use Policy
  ac-20_odp.03:
    alt-identifier: ac-20_prm_3
    profile-param-value-origin: AC-20 implementation statement
    ssp-values:
    - not applicable - the establish-terms-and-conditions selection in part (a) is used
  ac-20_odp.04:
    alt-identifier: ac-20_prm_4
    profile-param-value-origin: AC-20 implementation statement
    ssp-values:
    - not applicable - AC-20 is satisfied through part (a) terms and conditions; no external-system type is categorically prohibited
x-trestle-global:
  profile:
...
Terms and conditions for teachers and school administrators are defined for authorized system use. TLS 1.2+ is required for all connections, teachers and school administrators use multifactor authentication through the external school IdP. After authentication, users gain access to their role-based functions. Organization data remains within the application session and may not be exported or stored, according to IPS Acceptable Use Policy.

System admins are authorized to access the system exclusively through the bastion path described in AC-17. All other forms of privileged access are prohibited.
